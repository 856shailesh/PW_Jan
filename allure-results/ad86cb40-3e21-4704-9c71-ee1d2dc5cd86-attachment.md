...
- Location: tests/loginpage.spec.ts:24:1
        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            - generic [ref=e92]:
              ...
              - textbox "E-Mail Address" [ref=e94]: 856shaileshkumar@gmail.com
            - generic [ref=e95]:
              - generic [ref=e96]: Password
              - textbox "Password" [ref=e97]: IloveIndiaaaa
...
```ts
  ...
  12 | })
  13 | 
  14 | test('login page title test', async ({ }) => {
  15 |     const pageTitle = await loginPage.getLoginPageTitle();
  16 |     console.log("login page title is , ", pageTitle);
  17 |     expect(pageTitle).toBe('Account Login');
  18 | })
  19 | 
  20 | test('forget password link exist test', async ({ }) => {
  21 |     expect(await loginPage.isForgetPwdLinkExist()).toBeTruthy(); // Assert
  22 | })
  23 | 
  24 | test('user is able to login to app test', async ({ }) => {
  25 |     await loginPage.doLogin('856shaileshkumar@gmail.com', 'IloveIndiaaaa');
  26 |     expect.soft(await homePage.isLogoutLinkExist()).toBeTruthy();
> 27 |     expect.soft(await homePage.getHomePageTitle()).toBe('My Account');
     |                                                    ^ Error: expect(received).toBe(expected) // Object.is equality
  28 | })
```